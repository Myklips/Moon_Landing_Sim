/* autogenerated by Processing revision 1293 on 2024-05-01 */
...
public class Rocket_Landing_Simulation extends PApplet {

PShape rocket;  //<>//

int terrainSize = 257; // Increase the size of the terrain grid
float[][] terrain = new float[terrainSize][terrainSize];
float roughness = 0.4f; // Reduce the roughness for a smoother terrain
...
public void draw() {
  ...
  pushMatrix();
  translate(0, rocketY - rocket.getHeight() * 0.5f, 0); 
  ...
  if (!landed && rocketY < getTerrainHeight(0, 0) + rocket.getHeight() * 0.5f - 3) {
    if (thrustersActive) {
...
public float getTerrainHeight(float x, float z) {
  int terrainX = PApplet.parseInt(map(x, -width/2, width/2, 0, terrainSize - 1));
  int terrainZ = PApplet.parseInt(map(z, -height/2, height/2, 0, terrainSize - 1));
...
public PVector calculateTerrainNormal(float x, float z) {
...
public void rotateTerrainNormal(PVector terrainNormal) {
  float rotationAngle = PVector.angleBetween(new PVector(0, 1, 0), terrainNormal);
  float softenedRotationAngle = rotationAngle * 0.3f;
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Rocket_Landing_Simulation" };
    if (passedArgs != null) {
